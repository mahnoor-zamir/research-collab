...
export const SidebarTopNav = ({
  ...
  return (
    <section className="mb-2 flex flex-row justify-center gap-1">
      <SidebarTopNavItem sidebarNav={sidebarNav} value={home} navigate={() => setSidebarNav(home)}>
        <IconCube size={18} />
      </SidebarTopNavItem>
      <>
        ...
        {import.meta.env.VITE_API_WITH_FULL_FUNCTIONALITY && (
          <>
              ...
              <div className="relative">
                ...
                {currentUser.friends.some(
                  (friend: IUser["friends"][0]) =>
                    friend?.chat?.users?.find((user) => user.user === currentUser._id)?.isRead ===
                      false && userChat !== friend?.chat._id,
                ) && (
            ...
            >
              <div className="relative">
                <IconInbox size={18} />
                {currentUser.inbox?.filter((inbox: any) => inbox?.notificationRead === false)
                  .length > 0 && (
                  <div className="absolute -right-1 -top-1 h-2 w-2 rounded-full bg-fuchsia-500" />
                )}
              </div>
